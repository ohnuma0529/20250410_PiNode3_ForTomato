...
def dsw_read(pi):
    ...
    dsw1 = ~dsw1 & 0x01: 読み取ったスイッチの状態 (dsw1) を反転
                         (ON=1,OFF=0)とする
...
def sht85_read(pi):
    ...
    txd = bytearray([0x24, 0x00]): センサへの送信コマンドを指定
                                   コマンド(0x24, 0x00)は測定開始を表す
...
def mcp3204_read(pi, ch):
    ...
    txd = (0x06 | (ch > 2), 0x00 | (ch << 6), 0x00): MCP3204に対して適切なチャネルを指定するための3バイトのSPIコマンドを準備
        ...
        0x00 | (ch << 6): 2バイト目
                          ch = ch & 0x07の操作によりchは左づめの3bitに意味を持つビットが存在する状態にある.
                          実際はchは0～3の値であり,意味を持つ値は2ビット分である
                          したがって6ビットだけchをシフトさせることで意味を持つビットをxferメソッド指定するビット位置に割り当てできる
                          最後に0x00との or演算を行うことで必要ビット以外のビット位置を0とし,指定フォーマットのバイトを作成する
        0x00: 3バイト目: xferメソッド実行時に必要であるが特別送信するデータがないためダミービットを指定
         
    上記で作成したtxdを用いてspi通信を実行
